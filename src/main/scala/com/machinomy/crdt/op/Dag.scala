...
/**
  ...
  */
case class Dag[A, V <: VertexLike[A], E <: EdgeLike[A, V]](state: GraphLike[A, V, E]) {
  def contains(v: V) = state.vertices.contains(v)
  def contains(e: E) = state.edges.contains(e)

  def add(e: E): Dag.UpdateResult[A, V, E] =
    if(contains(e.u) && contains(e.v) && state.existsPath(e.u, e.v)) {
      val next = Dag(state.add(e))
      (next, Some(Dag.AddEdge[A, V, E](e)))
    } else {
      (this, None)
    }

  def add(v: V, left: V, right: V)(implicit eb: CanBuildEdge[A, V, E]): Dag.UpdateResult[A, V, E] =
    if (!contains(v) && contains(left) && contains(right) && state.existsPath(left, right)) {
      ...
      val next = Dag(state)
      (next, Some(Dag.AddVertex(v, left, right)))
    } else {
      (this, None)
    }
...
object Dag {
  type UpdateResult[A, V <: VertexLike[A], E <: EdgeLike[A, V]] = (Dag[A, V, E], Option[Update[A]])
